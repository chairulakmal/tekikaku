...
- **SemVer via release-please** (Google's GitHub Action, v17): it parses conventional commits, maintains a running release PR, and merging that PR creates the git tag, GitHub Release, and CHANGELOG entry. Chosen over semantic-release (heavier config, publishes immediately with no checkpoint) and changesets (built for multi-package repos and teams) as the lowest-maintenance fit for a solo single-repo app on the Actions free tier.
...
| release-please                                     | v17 action                                           | Release PR, tags, CHANGELOG                                                                                                                                                                                               |
